...
export function useAllProposalData(): { data: ProposalData[]; loading: boolean } {
  ...
  const gov0ProposalIndexes = useMemo(() => {
    return chainId === SupportedChainId.AVALANCHE ? V0_PROPOSAL_IDS : countToIndices(proposalCount0)
  }, [chainId, proposalCount0])
  const gov1ProposalIndexes = useMemo(() => {
    return chainId === SupportedChainId.AVALANCHE ? V1_PROPOSAL_IDS : countToIndices(proposalCount1)
